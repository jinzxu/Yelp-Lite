...
SiteSchema.virtual('properties.popUpMarkup').get(function () { //creating a virtual property to use data for Mapbox (but not stored in our mongo db)
    return `<strong><a href="/campgrounds/${this._id}">${this.title}</a></strong>
    <p>${this.description.substring(0, 30)}...</p>`
});

SiteSchema.post('findOneAndDelete', async function (doc) { //anytime a campground is deleted, all of the related reviews are also deleted
    console.log(doc);
