...
module.exports.index = async (req, res) => {
    const campgrounds = await Site.find({});
...
module.exports.createSite = async (req, res, next) => {
    ...
    const campground = new Site(req.body.campground);
...
module.exports.showSite = async (req, res) => {
    const campground = await Site.findById(req.params.id).populate({
        path: 'reviews',
        populate: {
            path: 'author'
        }
...
module.exports.renderEditForm = async (req, res) => {
    ...
    const campground = await Site.findById(req.params.id);
    if (!campground) {
        req.flash('error', 'Cannot find that campground!');
        return res.redirect('/campgrounds')
    }
...
module.exports.updateSite = async (req, res) => {
    ...
    const campground = await Site.findByIdAndUpdate(id, { ...req.body.campground }) //... spreads the object
